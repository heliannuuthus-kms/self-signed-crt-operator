...
import (
	...
	issuerutil "github.com/heliannuuthus/privateca-issuer/internal/issuer/util"
...
func (r *CertificateRequestReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	if certificateRequest.Spec.IssuerRef.Group != piv1alpha1api.GroupVersion.Group {
	...
	report := func(reason, message string, err error) {
		...
		r.recorder.Event(
			&certificateRequest,
			eventType,
			piv1alpha1api.EventReasonCertificateRequestReconciler,
			message,
		)
	...
	issuerGVK := piv1alpha1api.GroupVersion.WithKind(certificateRequest.Spec.IssuerRef.Kind)
	...
	ssigner, err := r.SignerBuilder(issuerSpec, secret.Data)
	if err != nil {
		return ctrl.Result{}, fmt.Errorf("%w: %v", errSignerBuilder, err)
	}

	signed, err := ssigner.Sign(certificateRequest.Spec.Request)
	if err != nil {
...
func (r *CertificateRequestReconciler) SetupWithManager(mgr ctrl.Manager) error {
	r.recorder = mgr.GetEventRecorderFor(piv1alpha1api.EventSource)
