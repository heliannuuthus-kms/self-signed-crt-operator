...
// ClusterCAIssuerReconciler reconciles a ClusterCAIssuer object
type ClusterCAIssuerReconciler struct {
	client.Client
	Log               logr.Logger
...
func (r *ClusterCAIssuerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("clustercaissuer", req.NamespacedName)
	iss := new(piv1alpha1api.ClusterCAIssuer)
	if err := r.Client.Get(ctx, req.NamespacedName, iss); err != nil {
		log.Error(err, "Failed to request ClusterCAIssuer")
...
func (r *ClusterCAIssuerReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&piv1alpha1api.ClusterCAIssuer{}).
		Complete(r)
}
